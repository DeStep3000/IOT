...
void Polygon::input_from_file(const std::string &input,
                              std::size_t index) {//add vertices in Polygon from file using input_from_array
...
bool Polygon::is_point_in(Point point) { //checking is point inside polygon
    ...
    for (int i = 0; i < this->num_vertices; i++) {
        ...
        bool is_between = this->vertices[i].y < point.y && this->vertices[j].y > point.y ||
                          this->vertices[j].y < point.y && this->vertices[i].y > point.y;
        bool check_intersection = is_between && (this->vertices[i].x + (point.y - this->vertices[i].y) /
...
bool Polygon::is_point_in_2(Point p) {//this method use angle sum
    ...
    for (int i = 0; i < this->num_vertices; i++) {
        ...
        if (vec_mult > 0) {
            result += acos(((this->vertices[i] - p) * (this->vertices[j] - p)) / length(this->vertices[i] - p) /
                           length(this->vertices[j] - p));
        } else if (vec_mult < 0) {
            result -= acos(((this->vertices[i] - p) * (this->vertices[j] - p)) / length(this->vertices[i] - p) /
                           length(this->vertices[j] - p));
        }
...
bool PolygonField::is_intersected(Point a, Point b, Point c,
...
std::vector<Point> PolygonField::intersection(Point a, Point b, Point c,
                                              Point d) {//func to find point of intersection of AB and CD ПЕРЕПИСАТЬ
    Point res{0, 0};
...
Polygon PolygonField::intersect_polygons(Polygon &p1, Polygon &p2) { //intersection of 2 Polygons
    ...
    if (!check_p1_full_out && !check_p2_full_out &&
        intersection_points.empty()) {//return blank in case of no intersection
        return {};
    }
